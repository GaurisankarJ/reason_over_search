...
class SearchR1Reward(BaseMetric):
    metric_name = "search_r1_reward"
    ...
    def calculate_metric(self, data):
        ...
        for item in data:
            reward_scores.append(float(getattr(item, "search_r1_reward", 0.0)))
            format_valid_scores.append(1.0 if bool(getattr(item, "search_r1_format_valid", False)) else 0.0)
            retrieval_hit_scores.append(1.0 if bool(getattr(item, "search_r1_retrieval_hit", False)) else 0.0)

        if len(reward_scores) == 0:
            return {
                "search_r1_reward": 0.0,
                "search_r1_format_valid_rate": 0.0,
                "search_r1_retrieval_hit_rate": 0.0,
            }, []

        return {
            "search_r1_reward": float(np.mean(reward_scores)),
            "search_r1_format_valid_rate": float(np.mean(format_valid_scores)),
            "search_r1_retrieval_hit_rate": float(np.mean(retrieval_hit_scores)),
        }, reward_scores

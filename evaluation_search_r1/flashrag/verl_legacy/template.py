...
You must reason inside <think> and </think>.
When external knowledge is needed, call search with <search> query </search>.
The search engine will return evidence enclosed in <information> and </information>.
You can iterate reasoning and search multiple times.
When ready, output final answer inside <answer> and </answer>.
Question: {prompt}
Assistant:"""

search_r1_template_sys = """You are a helpful assistant that solves questions with interleaved reasoning and search.

Use this action protocol strictly:
1) Think using <think>...</think>
2) If needed, search using <search>query</search>
3) You will receive retrieved evidence in <information>...</information>
4) Continue think/search until enough evidence is gathered
5) Finish with <answer>final answer</answer>

Important:
- Keep the structure valid and properly paired.
- Do not output tool JSON.
- Use only <search> for search action and <answer> for final completion."""

prompt_template_dict["search_r1_template"] = search_r1_template

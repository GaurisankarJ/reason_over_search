...
def main():
    ...
    # Example trajectories from sample_step
    sample_file = exp_dir / f"train_data_step{sample_step}.jsonl"
    if sample_file.exists():
        sample_rows = []
        with open(sample_file) as fp:
            for line in fp:
                sample_rows.append(parse_trajectory(json.loads(line)))

        print(f"**Example trajectories from step {sample_step}** (2 sampled, ~300 lines each):")
        print()

        # Highest reward
        best = max(sample_rows, key=lambda r: r["reward"])
        if best["reward"] > 0:
            print(format_example(best, f"best reward in step {sample_step}"))
        else:
            print(f"_(no nonzero-reward trajectories in step {sample_step}; showing two zero-reward representative examples below)_")
            print()

        # First zero-reward (or second-best if all zero)
        zero = next((r for r in sample_rows if r["reward"] == 0.0), sample_rows[0])
        if zero is not best:
            print(format_example(zero, f"representative zero-reward in step {sample_step}"))

...
#
# Usage:
#   bash training_m7_1/scripts/upload_ckpts_watcher.sh [--mode prod|smoke] [--seed N]
#
# Defaults: --mode prod --seed 42. Reads HF_TOKEN + HF_REPO_PREFIX from
# training_m7_1/.env (gitignored). Each step N is uploaded to:
...
if [[ -z "${HF_TOKEN:-}" || -z "${HF_REPO_PREFIX:-}" ]]; then
    echo "error: HF_TOKEN and HF_REPO_PREFIX must be set in training_m7_1/.env" >&2
    exit 2
fi
...
upload_step() {
    ...
    local repo_name="${HF_REPO_PREFIX}-m5_${MODE}-seed${SEED}-step${step_n}"
...
- Run config: `training_m7_1/configs/m7_1_research_paper.yaml`

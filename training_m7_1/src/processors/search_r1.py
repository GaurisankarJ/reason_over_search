...
from training_m7_1.src.chat_template.tools import SEARCH_TOOL
...
def search_r1_processor(
    ...
    repo's two dataset-prep scripts:
      ...
      - M5.1 MuSiQue : `training_m7_1/scripts/prep_musique.py`
    ...
    elif arm == "qwen_native_no_system":
        # M7 arm: byte-aligned to the M4.3 base lock (qwen35_minimal_no_system).
        # Identical to `qwen_native` except `tools=[SEARCH_TOOL]` is NOT passed
        # to apply_chat_template, so the chat template emits NO system block
        # at all. The format spec (<tool_call>...</tool_call> schema) must
        # therefore be inlined into the user prompt (the prompt file handles
        # this; see training_m7_1/src/prompts/m7_qwen35_base_user.txt).
        # M4.3 evidence (RESULTS_SMOKE_m4 §7.4): auto-inject hurts base 5x
        # because base lacks the tool-use post-training prior, so the
        # `# Tools` schema + `<IMPORTANT>` reminder is scaffolding noise.
        if task_data_spec.prompt is None:
            raise ValueError(
                "qwen_native_no_system arm requires task_data_spec.prompt_file to be set "
                "(typically training_m7_1/src/prompts/m7_qwen35_base_user.txt)."
            )
        user_content = task_data_spec.prompt.format(question)
        convo.append({"role": "user", "content": user_content})
        rendered = tokenizer.apply_chat_template(
            convo,
            # NO tools= argument — this is the load-bearing difference vs qwen_native
            tokenize=False,
            add_generation_prompt=True,
            add_special_tokens=False,
            enable_thinking=True,  # match M4.3 lock render shape (open <think>\n)
        )
    else:

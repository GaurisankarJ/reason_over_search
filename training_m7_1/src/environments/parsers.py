...
For the `qwen_native` arm the action parser is **re-exported from the M4 eval
...
VALID_ARMS = ("qwen_native", "qwen_native_no_system", "paper")

# Inject `evaluation_qwen35/` so `flashrag.search_r1.*` resolves; mirrors the
# same trick used in src/rewards/search_r1.py (see the comment there). Both
# overlays must agree on which `flashrag` they pull in.
...
from flashrag.search_r1.parser import extract_tool_call_query  # noqa: E402
...
def parse_query(arm: str, assistant_text: str) -> Optional[str]:
    ...
    For `qwen_native`, delegates to the M4-shared
    `flashrag.search_r1.parser.extract_tool_call_query`, which returns a
    `(query, reason)` tuple; we collapse to `Optional[str]` (the env doesn't
    need the reason here, only the query body).
    """
    if arm in ("qwen_native", "qwen_native_no_system"):
        # Both qwen_native arms emit the same Qwen3.5 nested-XML <tool_call>
        # format — only the render-side system block differs (auto-inject ON
        # for qwen_native, OFF for qwen_native_no_system / M7 base arm).
        query, reason = extract_tool_call_query(assistant_text)
        return query if reason == "valid_tool_call_tag" else None
    if arm == "paper":

...
sweep locks (mandated by docs/milestone_5/MILESTONE_5_5.md §"Prompt + tag
...
Examples (run from the repo root):
    ...
    python training_m5_5/scripts/sync_m4_prompts.py --mode qwen35_recall_port

    # List available modes:
    python training_m5_5/scripts/sync_m4_prompts.py --list
...
OUT_DIR = REPO_ROOT / "training_m5_5" / "src" / "prompts"
...
def main() -> int:
    ...
    print(f"  data.train.prompt_file: training_m5_5/src/prompts/{USER_TXT.name}")
    if system_text is not None:
        print(f"  data.train.system_prompt_file: training_m5_5/src/prompts/{SYSTEM_TXT.name}")
    else:

...
#
# Examples:
#   bash training_m5_5/scripts/run.sh --mode smoke
#   bash training_m5_5/scripts/run.sh --mode smoke --seed 7
#   bash training_m5_5/scripts/run.sh --mode prod  --seed 42
#   bash training_m5_5/scripts/run.sh --mode smoke -- policy.train_micro_batch_size=8
#
# Prerequisites:
#   1. Retriever live at 127.0.0.1:3005 (curl -sS http://127.0.0.1:3005/health → healthy)
#   2. training_m5_5/.env populated (WANDB_API_KEY); copy from .env.example if missing.
#   3. training_m5_5/nemo_rl/.venv materialized (training_m5_5/setup.sh or uv sync).
#   4. data/training/musique/train.parquet present
#      (training_m5_5/scripts/prep_musique.py downloads + reshapes it).
#   5. training_m5_5/src/prompts/m5_qwen35_user.txt populated
#      (training_m5_5/scripts/sync_m4_prompts.py --mode <M4 winner>; pre-staged
...
VENV_PYTHON="${REPO_ROOT}/training_m5_5/nemo_rl/.venv/bin/python"
if [[ ! -x "${VENV_PYTHON}" ]]; then
    echo "error: venv not found at ${VENV_PYTHON}" >&2
    echo "       run training_m5_5/setup.sh first" >&2
    exit 1
fi

if [[ -f "${REPO_ROOT}/training_m5_5/.env" ]]; then
    set -a; source "${REPO_ROOT}/training_m5_5/.env"; set +a
fi
...
exec "${VENV_PYTHON}" training_m5_5/scripts/run_grpo.py \
    --config="${CONFIG}" \
    "${OVERRIDES[@]}" \
    "${EXTRA_OVERRIDES[@]}"

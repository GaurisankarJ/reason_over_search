...
9. [x] Side-by-side M3 vs M3.1 comparison (pre-GRPO / z7kcxfof / el6s2d2h) populated in RESULTS_v2 §14.4-§14.5 and supervisor brief §4.8.
10. [ ] Training-plot comparison panel (with-example z7kcxfof vs no-example el6s2d2h: reward, tool calls, num_turns, response_length curves) — already-trained data, only the visualisation is new. Tracked in [`docs/report/SUPERVISOR_MEETING_2026-05-07.md`](../report/SUPERVISOR_MEETING_2026-05-07.md) §4.8 plan. **Carry-over to follow-up; not blocking M3.1 close-out.**

...
- **v0 best (no example)** — `p3_decide_no_ex` (W&B id `el6s2d2h`, blue solid): tool-use survives example removal because the rules section has explicit per-step decision guidance. End-of-run reward **0.215** at 2280 steps, the highest of v0. Checkpoints archived at [`docs/archive/verl_runs/v0/p3_decide_no_ex_el6s2d2h/`](../archive/verl_runs/v0/p3_decide_no_ex_el6s2d2h/) (steps 500 / 1000 / 1500 / 2000).
- **v0 best with example** — `p1_basic_w_ex` (W&B id `z7kcxfof`): the only v0 run that converged on **heavy-tool 2-call / 4-turn** behaviour (response length climbed 1788 → 2047 tokens; reward 0.148 → **0.190**, +28 % rel over 1046 steps). The 2-search Hamlet example anchors "always do 2 searches". This is the checkpoint M3 evaluates — preserved at [`docs/archive/verl_runs/v0/p1_basic_w_ex_z7kcxfof/`](../archive/verl_runs/v0/p1_basic_w_ex_z7kcxfof/) and [`eval/qwen_3_0.6b_v0/`](../../eval/qwen_3_0.6b_v0/).
...
**Status**: first attempt sbatch **2134645** failed at SGLang `/health`-readiness wait (300 s budget — M3 reference was 260 s, this one cold-cache side of the cliff; logs: `logs/m3_2134645_*.{out,err,log}`). Bumped wait window 300 → 600 s in `scripts/sbatch_m3.sh` and re-submitted as **2134663** (gpu-short, 4 h). Logs: `logs/m3_2134663_*.{out,err,log}`. Will run all 7 datasets autonomously, ~2.5 h expected. Numerical results land in [`RESULTS_v2.md`](RESULTS_v2.md) §14. Milestone narrative: [`../milestone_three/MILESTONE_3.1.md`](../milestone_three/MILESTONE_3.1.md). Setup diff: [`CODE_SETUP_v2.md`](CODE_SETUP_v2.md) §13.5.

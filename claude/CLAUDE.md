...
**Milestone 8 (M8) — Chain-consistency + retrieval-grounded reward extensions** — **scoped 2026-05-16, blocked on M5.1 finishing**. Acts on the M5.1 cadence 6-9 plateau (window mean 0.22-0.24 across 40 steps) by adding two reward components, both pure functions of the rollout string: (A) chain-consistency penalty on silent `<think>` entity flips not justified by intervening `<tool_response>`; (B) retrieval-grounded factor on how much of the predicted answer's tokens appear in retrieved chunks. Composed: `reward = f1 * (1 - chain_inconsistency) * max(0.3, retrieval_grounded)`. ~50 LoC in `training_m8_2/src/rewards/search_r1.py`, copied from `training_m5_1/`. Trace evidence for the structural ceiling (Fox Island silent-flip vs Kotri clean-chain, both reward 1.0 under M5.1) in [`report/RESULTS_M5_1_H200.md §9.5`](../docs/report/RESULTS_M5_1_H200.md). Predicted M8.2 ceiling 0.27-0.32 vs M5.1's 0.22-0.24; negative-result outcome (no lift) constrains future work to model-prior scaling. Total budget ~$200. Doc trail: [`milestone_8/MILESTONE_8.md`](../docs/milestone_8/MILESTONE_8.md).

...
from flashrag.search_r1.templates import (
    SEARCH_R1_TEMPLATE,
    QWEN3_0_6B_TEMPLATE,
    QWEN3_TEMPLATES,
...
def _is_qwen3(mode: str) -> bool:
    ...
    return mode == 'qwen3' or mode.startswith('qwen3_')


def _is_qwen35(mode: str) -> bool:
...
class SearchR1Pipeline(BasicPipeline):
    def __init__(self, config, retriever=None, generator=None, apply_chat=False, enable_thinking=False, prompt_mode='search_r1'):
        ...
        # 'qwen3*' / 'qwen35*': system-message format; 'search_r1': original user-message format.
        # All qwen-family modes share retrieval-text format and per-mode budgets;
        # only the action / observation tags and the system message differ.
        if prompt_mode in QWEN3_TEMPLATES:
            self.prompt_template = QWEN3_TEMPLATES[prompt_mode]
        elif prompt_mode in QWEN35_TEMPLATES:
            self.prompt_template = QWEN35_TEMPLATES[prompt_mode]
        elif prompt_mode == 'search_r1':
            self.prompt_template = SEARCH_R1_TEMPLATE
        else:
            valid = sorted(set(QWEN3_TEMPLATES) | set(QWEN35_TEMPLATES)) + ['search_r1']
            raise ValueError(f"Unknown prompt_mode: {prompt_mode!r}; valid: {valid}")
    ...
    def run_item(self, item):
        ...
        # The action stop string is the closing tag of whatever scheme the prompt
        # uses. qwen35 emits `<tool_call>...</tool_call>`; qwen3 / search_r1 emit
        # `<search>...</search>`. SGLang stops on the first match in the list.
        if _is_qwen35(self.prompt_mode):
            action_stop = '</tool_call>'
        else:
            action_stop = '</search>'
        stop_tokens = [action_stop, '</answer>', '<|im_end|>', '<|endoftext|>']
        ...
        while remain_length > 0 and turns < max_search_turns:
            ...
            if stop_reason == 'stop' and isinstance(stop_matched, str) and action_stop in stop_matched:
                output_str = response['text']
                if action_stop in stop_matched and action_stop not in output_str:
                    output_str += action_stop

                if _is_qwen35(self.prompt_mode):
                    search_content, search_status = extract_tool_call_query(output_str)
                    valid_prefix = "valid_tool_call"
                else:
                    search_content, search_status = extract_search_tag_query(output_str)
                    valid_prefix = "valid_search"

...
def search_r1(args, config_dict):
    ...
    # All qwen3* modes (M3 / M3.1) were trained with top_n=5; the qwen35* M4
    # baseline matches that for cross-family comparability. search_r1 stays at
    # top-3 (paper fidelity).
    if args.prompt_mode.startswith("qwen3"):  # matches both 'qwen3*' and 'qwen35*'
        config_dict["retrieval_topk"] = 5
    config = Config(args.config_path, config_dict)
    # FlashRAG's Dataset uses random.sample (global random) for subsampling but
    # never seeds it. Seed here so the 100-item quick-eval set is deterministic
    # across runs of the same config.seed (FlashRAG default 2024).
...
if __name__ == "__main__":
    ...
    # for the M4 quick eval; leave both at defaults for the full sweep.
    parser.add_argument("--test_sample_num", type=int, default=None,
                        help="If set, subsample this many items from the split (FlashRAG test_sample_num)")
    parser.add_argument("--random_sample", type=_str2bool, default=False,
                        help="If True, random.sample N items; otherwise take the first N (FlashRAG random_sample)")
    parser.add_argument("--seed", type=int, default=None,
                        help="Override Config seed (FlashRAG default 2024); affects random_sample shuffling")
    ...
    if args.test_sample_num is not None:
        ...
        config_dict["random_sample"] = args.random_sample
